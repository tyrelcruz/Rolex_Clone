...
function filterProducts(filterValue) {
    ...
    // Log the filter value for debugging purposes
    console.log(`Filtering with category: ${filterValue}`);

    allProducts.forEach((product) => {
        const gender = product.getAttribute('data-gender');  // Get the category (men/women)
        console.log(`Filtering product: ${product.innerHTML}, Category: ${gender}`); // Log the gender for each product

        // Determine if the product should be visible based on the filter value
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (menTab) {
        menTab.addEventListener('click', () => {
            console.log('Filter by Men clicked');  // Debugging log
            filterProducts('men');
        });
    }
    ...
    if (womenTab) {
        womenTab.addEventListener('click', () => {
            console.log('Filter by Women clicked');  // Debugging log
    ...
    if (allTab) {
        allTab.addEventListener('click', () => {
            console.log('Filter by All clicked');  // Debugging log
            filterProducts('all');
        });
    }
});
